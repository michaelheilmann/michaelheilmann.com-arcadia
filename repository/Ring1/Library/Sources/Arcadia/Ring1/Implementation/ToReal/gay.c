  ...
  )
{
  ...
  // Decimal digits beyond 19 are not added to the significand. However, for each such digits, the
  ...
  //Arcadia_SizeValue numberOfIntegralDigits = source->significand.integral.length - source->significand.integral.leadingZeroes.length;
  ...
  )
{
  return info->roundNearestTieEven.isAbove
      ...
      || (info->roundNearestTieEven.isOd && info->roundNearestTieEven.isHalfway);
}
  ...
  )
{
  if (info->roundNearestTieEven.order > 0) {
    ...
    return info->roundNearestTieEven.isOd;
  }
}
  ...
  )
{
  ...
  info->roundNearestTieEven.isOd = isOdd;
