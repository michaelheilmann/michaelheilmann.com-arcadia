...
#include "Tools/Visuals2d/ImageWriter.h"
...
#define R_Status_EnvironmentFailed (2000)
  ...
  )
{
  ...
  size_t sourceWidth = PixelBuffer_getWidth(sourcePixelBuffer);
  size_t sourceHeight = PixelBuffer_getHeight(sourcePixelBuffer);
...
void writeIconToByteBuffer(R_List* sourcePixelBuffers, R_ByteBuffer* targetByteBuffer) {
  ...
  for (R_SizeValue i = 0, offset = 0, n = R_List_getSize(sourcePixelBuffers); i < n; ++i) {
    ...
    if (PixelFormat_An8Rn8Gn8Bn8 != PixelBuffer_getFormat(pixelBuffer)) {
      R_setStatus(R_Status_ArgumentValueInvalid);
      R_jump();
    }
    size_t width = PixelBuffer_getWidth(pixelBuffer),
           height = PixelBuffer_getHeight(pixelBuffer);
    if (width > 256) {
      R_setStatus(R_Status_ArgumentValueInvalid);
      R_jump();
    }
