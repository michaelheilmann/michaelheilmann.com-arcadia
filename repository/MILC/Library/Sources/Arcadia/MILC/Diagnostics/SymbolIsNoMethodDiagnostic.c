...
constructImpl
  (
    ...
    Arcadia_MILC_Diagnostics_SymbolIsNoMethodDiagnostic* self
  );
...
initializeDispatchImpl
  (
    Arcadia_Thread* thread,
    Arcadia_MILC_Diagnostics_SymbolIsNoMethodDiagnosticDispatch* self
  );
...
Arcadia_defineObjectType(u8"Arcadia.MILC.Diagnostics.SymbolIsNoMethodDiagnostic", Arcadia_MILC_Diagnostics_SymbolIsNoMethodDiagnostic,
...
{
  Arcadia_EnterConstructor(Arcadia_MILC_Diagnostics_SymbolIsNoMethodDiagnostic);
  ...
  Arcadia_LeaveConstructor(Arcadia_MILC_Diagnostics_SymbolIsNoMethodDiagnostic);
}
  ...
  )
{
  ...
  Arcadia_StringBuilder_insertBackCxxString(thread, stringBuffer, u8"` is no method\n");
  return Arcadia_String_create(thread, Arcadia_Value_makeObjectReferenceValue(stringBuffer));
}

Arcadia_MILC_Diagnostics_SymbolIsNoMethodDiagnostic*
...
{ 
  _Arcadia_BeginCreate(Arcadia_MILC_Diagnostics_SymbolIsNoMethodDiagnostic);
  ...
  _Arcadia_EndCreate(Arcadia_MILC_Diagnostics_SymbolIsNoMethodDiagnostic);
}
