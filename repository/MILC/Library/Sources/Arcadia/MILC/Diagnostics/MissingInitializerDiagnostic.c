...
constructImpl
  (
    ...
    Arcadia_MILC_Diagnostics_MissingInitializerDiagnostic* self
  );
...
initializeDispatchImpl
  (
    Arcadia_Thread* thread,
    Arcadia_MILC_Diagnostics_MissingInitializerDiagnosticDispatch* self
  );
...
Arcadia_defineObjectType(u8"Arcadia.MILC.Diagnostics.MissingInitializerDiagnostic", Arcadia_MILC_Diagnostics_MissingInitializerDiagnostic,
...
{
  Arcadia_EnterConstructor(Arcadia_MILC_Diagnostics_MissingInitializerDiagnostic);
  ...
  self->symbol = (Arcadia_MILC_EnumerationConstantSymbol*)Arcadia_ValueStack_getObjectReferenceValueChecked(thread, 1, _Arcadia_MILC_EnumerationConstantSymbol_getType(thread));
  Arcadia_LeaveConstructor(Arcadia_MILC_Diagnostics_MissingInitializerDiagnostic);
}
  ...
  )
{
  ...
  Arcadia_StringBuilder_insertBackCxxString(thread, stringBuilder, u8"error: missing initializer for enumeration constant `");
...
{ 
  _Arcadia_BeginCreate(Arcadia_MILC_Diagnostics_MissingInitializerDiagnostic);
  ...
  _Arcadia_EndCreate(Arcadia_MILC_Diagnostics_MissingInitializerDiagnostic);
}
