...
#define A2 &(arguments[1])
  ...
  if (!Arcadia_Type_isSubType(thread, Arcadia_Object_getType(other1), _Arcadia_String_getType(thread))) {
    Arcadia_Value_setBooleanValue(target, Arcadia_BooleanValue_False);
    return;
  }
  ...
  )
{
  ...
  return !Arcadia_Process_compareMemory(Arcadia_Thread_getProcess(thread),
  ...
  )
{
  ...
  Arcadia_isUtf8(Arcadia_Thread_getProcess(thread), Arcadia_String_getBytes(thread, self), Arcadia_String_getNumberOfBytes(thread, self), &numberOfSymbols);
...
{
  if (Arcadia_ImmutableUtf8String_getNumberOfBytes(thread, self->immutableUtf8String) == numberOfBytes) {
    return !Arcadia_Process_compareMemory(Arcadia_Thread_getProcess(thread), Arcadia_ImmutableUtf8String_getBytes(thread, self->immutableUtf8String),
                                          bytes, numberOfBytes);
  } else {
    return Arcadia_BooleanValue_False;
  }
