  ...
  )
{
  ...
  if (Arcadia_JumpTarget_save(&jumpTarget)) {
    ...
    R_executeProcedure(Arcadia_Thread_getProcess(thread), interpreterProcess, R_Interpreter_Procedure_create(thread, Arcadia_String_create_pn(thread, Arcadia_RuntimeByteArray_create(thread, u8"main", sizeof(u8"main") - 1)), code));
  ...
  )
{
  ...
  if (Arcadia_JumpTarget_save(&jumpTarget)) {
    ...
    if (1 != R_Interpreter_Code_Constants_getOrCreateString(Arcadia_Process_getThread(process), constants, Arcadia_String_create_pn(thread, Arcadia_RuntimeByteArray_create(thread, u8"Hello, World!\n", sizeof(u8"Hello, World!\n"))))) {
      Arcadia_Thread_setStatus(thread, Arcadia_Status_TestFailed);
      Arcadia_Thread_jump(thread);
    }
    ...
    R_executeProcedure(process, interpreterProcess, R_Interpreter_Procedure_create(thread, Arcadia_String_create_pn(thread, Arcadia_RuntimeByteArray_create(thread, u8"main", sizeof(u8"main") - 1)), code));
