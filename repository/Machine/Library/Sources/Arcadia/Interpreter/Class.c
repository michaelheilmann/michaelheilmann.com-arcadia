...
{
  _Arcadia_BeginCreate(R_Interpreter_Class);
  if (className) {
  ...
  _Arcadia_EndCreate(R_Interpreter_Class);
}
...
{
  Arcadia_String* name = Arcadia_String_create_pn(Arcadia_Process_getThread(process), Arcadia_RuntimeByteArray_create(Arcadia_Process_getThread(process), u8"<constructor>", sizeof(u8"<constructor>") - 1));
...
{
  Arcadia_Value key = Arcadia_Value_makeObjectReferenceValue(method->unqualifiedName);
  Arcadia_Value value = Arcadia_Map_get(Arcadia_Process_getThread(process), self->classMembers, key);
  if (!Arcadia_Value_isVoidValue(&value)) {
    Arcadia_Thread_setStatus(Arcadia_Process_getThread(process), Arcadia_Status_Exists);
    Arcadia_Thread_jump(Arcadia_Process_getThread(process));
...
{
  Arcadia_Value key = Arcadia_Value_makeObjectReferenceValue(variable->name);
  Arcadia_Value value = Arcadia_Map_get(Arcadia_Process_getThread(process), self->classMembers, key);
  if (!Arcadia_Value_isVoidValue(&value)) {
    Arcadia_Thread_setStatus(Arcadia_Process_getThread(process), Arcadia_Status_Exists);
    Arcadia_Thread_jump(Arcadia_Process_getThread(process));
  ...
  )
{
  ...
  if (self->extendedClassName) {
    ...
    do {
      ...
      Arcadia_Value v = Arcadia_Value_makeObjectReferenceValue(self);
