// The author of this software is Michael Heilmann (contact@michaelheilmann.com).
//
// Copyright(c) 2024-2026 Michael Heilmann (contact@michaelheilmann.com).
//
// Permission to use, copy, modify, and distribute this software for any
// purpose without fee is hereby granted, provided that this entire notice
// is included in all copies of any software which is or includes a copy
// or modification of this software and in all copies of the supporting
// documentation for such software.
//
// THIS SOFTWARE IS BEING PROVIDED "AS IS", WITHOUT ANY EXPRESS OR IMPLIED
// WARRANTY.IN PARTICULAR, NEITHER THE AUTHOR NOR LUCENT MAKES ANY
// REPRESENTATION OR WARRANTY OF ANY KIND CONCERNING THE MERCHANTABILITY
// OF THIS SOFTWARE OR ITS FITNESS FOR ANY PARTICULAR PURPOSE.

#include "Arcadia/Interpreter/Variable.h"

#include "Arcadia/Interpreter/Include.h"

static void
R_Interpreter_Variable_constructImpl
  (
    Arcadia_Thread* thread,
    R_Interpreter_Variable* self
  );

static void
R_Interpreter_Variable_initializeDispatchImpl
  (
    Arcadia_Thread* thread,
    R_Interpreter_VariableDispatch* self
  );

static void
R_Interpreter_Variable_visit
  (
    Arcadia_Thread* thread,
    R_Interpreter_Variable* self
  );

static const Arcadia_ObjectType_Operations _objectTypeOperations = {
  Arcadia_ObjectType_Operations_Initializer,
  .construct = (Arcadia_Object_ConstructCallbackFunction*)&R_Interpreter_Variable_constructImpl,
  .visit = (Arcadia_Object_VisitCallbackFunction*)&R_Interpreter_Variable_visit,
  .initializeDispatch = (Arcadia_ObjectDispatch_InitializeCallbackFunction*)&R_Interpreter_Variable_initializeDispatchImpl,
};

static const Arcadia_Type_Operations _typeOperations = {
  Arcadia_Type_Operations_Initializer,
  .objectTypeOperations = &_objectTypeOperations,
};

Arcadia_defineObjectType(u8"R.Interpreter.Variable", R_Interpreter_Variable,
                         u8"Arcadia.Object", Arcadia_Object,
                         &_typeOperations);

static void
R_Interpreter_Variable_constructImpl
  (
    Arcadia_Thread* thread,
    R_Interpreter_Variable* self
  )
{
  Arcadia_EnterConstructor(R_Interpreter_Variable);
  {
    Arcadia_ValueStack_pushNatural8Value(thread, 0);
    Arcadia_superTypeConstructor(thread, _type, self);
  }
  if (2 != _numberOfArguments) {
    Arcadia_Thread_setStatus(thread, Arcadia_Status_NumberOfArgumentsInvalid);
    Arcadia_Thread_jump(thread);
  }
  self->ready = Arcadia_BooleanValue_False;
  self->index = Arcadia_SizeValue_Literal(0);
  self->class = Arcadia_ValueStack_getObjectReferenceValueChecked(thread, 2, _R_Interpreter_Class_getType(thread));
  self->name = Arcadia_ValueStack_getObjectReferenceValueChecked(thread, 1, _Arcadia_String_getType(thread));
  Arcadia_LeaveConstructor(R_Interpreter_Variable);
}

static void
R_Interpreter_Variable_initializeDispatchImpl
  (
    Arcadia_Thread* thread,
    R_Interpreter_VariableDispatch* self
  )
{/*Intentionally empty.*/}

static void
R_Interpreter_Variable_visit
  (
    Arcadia_Thread* thread,
    R_Interpreter_Variable* self
  )
{
  Arcadia_Object_visit(thread, (Arcadia_Object*)self->class);
  Arcadia_Object_visit(thread, (Arcadia_Object*)self->name);
}

R_Interpreter_Variable*
R_Interpreter_Variable_create
  (
    Arcadia_Thread* thread,
    R_Interpreter_Class* class,
    Arcadia_String* name
  )
{
  Arcadia_SizeValue oldValueStackSize = Arcadia_ValueStack_getSize(thread);
  if (class) {
    Arcadia_ValueStack_pushObjectReferenceValue(thread, class);
  } else {
    Arcadia_ValueStack_pushVoidValue(thread, Arcadia_VoidValue_Void);
  }
  if (name) {
    Arcadia_ValueStack_pushObjectReferenceValue(thread, name);
  } else {
    Arcadia_ValueStack_pushVoidValue(thread, Arcadia_VoidValue_Void);
  }
  Arcadia_ValueStack_pushNatural8Value(thread, 2);
  ARCADIA_CREATEOBJECT(R_Interpreter_Variable);
}

R_Interpreter_Class*
R_Interpreter_Variable_getClass
  (
    Arcadia_Thread* thread,
    R_Interpreter_Variable* self
  )
{ return self->class; }

Arcadia_String*
R_Interpreter_Variable_getName
  (
    Arcadia_Thread* thread,
    R_Interpreter_Variable* self
  )
{ return self->name; }
