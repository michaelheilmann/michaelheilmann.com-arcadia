...
#include "Arcadia/Visuals/Implementation/Windows/Wgl/WglFactory.h"
...
static void
Arcadia_Visuals_Windows_WglFactory_constructImpl
  (
    Arcadia_Thread* thread,
    Arcadia_Value* self,
...
static void
Arcadia_Visuals_Windows_WglFactory_destruct
  (
    Arcadia_Thread* thread,
...
static const Arcadia_ObjectType_Operations _objectTypeOperations = {
  .construct = &Arcadia_Visuals_Windows_WglFactory_constructImpl,
  .destruct = &Arcadia_Visuals_Windows_WglFactory_destruct,
  .visit = NULL,
};
...
Arcadia_defineObjectType(u8"Arcadia.Visuals.Windows.WglFactory", Arcadia_Visuals_Windows_WglFactory, u8"Arcadia.Object", Arcadia_Object, &_typeOperations);
  ...
  )
{
  ...
  if (self->window) {
    Arcadia_Visuals_Windows_WglFactoryWindow_close(thread, self->window);
    self->window = NULL;
  }
}
...
{
  Arcadia_Visuals_Windows_WglFactory* _self = Arcadia_Value_getObjectReferenceValue(self);
  Arcadia_TypeValue _type = _Arcadia_Visuals_Windows_WglFactory_getType(thread);
  {
    Arcadia_Value argumentValues[] = {
  ...
  )
{
  ...
  Arcadia_Visuals_Windows_WglFactory* self = Arcadia_allocateObject(thread, _Arcadia_Visuals_Windows_WglFactory_getType(thread), 0, &argumentValues[0]);
  return self;
}
  ...
  )
{
  ...
  self->window = Arcadia_Visuals_Windows_WglFactoryWindow_create(thread);
  if (Arcadia_JumpTarget_save(&jumpTarget)) {
    Arcadia_Visuals_Windows_WglFactoryWindow_open(thread, self->window);
...
static Arcadia_Integer32Value getIntegerAttribute(Arcadia_Thread* thread, Arcadia_Visuals_Windows_WglFactory* self, Arcadia_Visuals_Windows_WglFactoryWindow* targetWindow, int pixelFormatIndex, int attributeKey) {
  int attributeValue;
  ...
  )
{
  ...
  Arcadia_Visuals_Windows_WglFactoryWindow* window = Arcadia_Visuals_Windows_WglFactoryWindow_create(thread);
  Arcadia_JumpTarget jumpTarget;
  Arcadia_Thread_pushJumpTarget(thread, &jumpTarget);
  if (Arcadia_JumpTarget_save(&jumpTarget)) {
    ...
    for (Arcadia_Integer32Value majorVersion = 1; majorVersion < 5; majorVersion++) {
      for (Arcadia_Integer32Value minorVersion = 0; minorVersion < 9; minorVersion++) {
        Arcadia_Visuals_Windows_WglFactoryWindow_close(thread, window);
        Arcadia_Visuals_Windows_WglFactoryWindow_open(thread, window);
