// The author of this software is Michael Heilmann (contact@michaelheilmann.com).
//
// Copyright(c) 2024-2026 Michael Heilmann (contact@michaelheilmann.com).
//
// Permission to use, copy, modify, and distribute this software for any
// purpose without fee is hereby granted, provided that this entire notice
// is included in all copies of any software which is or includes a copy
// or modification of this software and in all copies of the supporting
// documentation for such software.
//
// THIS SOFTWARE IS BEING PROVIDED "AS IS", WITHOUT ANY EXPRESS OR IMPLIED
// WARRANTY.IN PARTICULAR, NEITHER THE AUTHOR NOR LUCENT MAKES ANY
// REPRESENTATION OR WARRANTY OF ANY KIND CONCERNING THE MERCHANTABILITY
// OF THIS SOFTWARE OR ITS FITNESS FOR ANY PARTICULAR PURPOSE.

#include "Arcadia/MIL/AST/ModuleNode.h"

#include "Arcadia/MIL/AST/Include.h"

/// @code
/// constructor()
/// @endcode
static void
Arcadia_MIL_AST_ModuleNode_constructImpl
  (
    Arcadia_Thread* thread,
    Arcadia_MIL_AST_ModuleNode* self
  );

static void
Arcadia_MIL_AST_ModuleNode_initializeDispatchImpl
  (
    Arcadia_Thread* thread,
    Arcadia_MIL_AST_ModuleNodeDispatch* self
  );

static void
Arcadia_MIL_AST_ModuleNode_visit
  (
    Arcadia_Thread* thread,
    Arcadia_MIL_AST_ModuleNode* self
  );

static const Arcadia_ObjectType_Operations _Arcadia_MIL_AST_ModuleNode_objectTypeOperations = {
  Arcadia_ObjectType_Operations_Initializer,
  .construct = (Arcadia_Object_ConstructCallbackFunction*)&Arcadia_MIL_AST_ModuleNode_constructImpl,
  .visit = (Arcadia_Object_VisitCallbackFunction*)&Arcadia_MIL_AST_ModuleNode_visit,
  .initializeDispatch = (Arcadia_ObjectDispatch_InitializeCallbackFunction*)&Arcadia_MIL_AST_ModuleNode_initializeDispatchImpl,
};

static const Arcadia_Type_Operations _Arcadia_MIL_AST_ModuleNode_typeOperations = {
  Arcadia_Type_Operations_Initializer,
  .objectTypeOperations = &_Arcadia_MIL_AST_ModuleNode_objectTypeOperations,
};

Arcadia_defineObjectType(u8"Arcadia.MIL.AST.ModuleNode", Arcadia_MIL_AST_ModuleNode,
                         u8"Arcadia.MIL.AST.Node", Arcadia_MIL_AST_Node,
                         &_Arcadia_MIL_AST_ModuleNode_typeOperations);

static void
Arcadia_MIL_AST_ModuleNode_constructImpl
  (
    Arcadia_Thread* thread,
    Arcadia_MIL_AST_ModuleNode* self
  )
{
  Arcadia_TypeValue _type = _Arcadia_MIL_AST_ModuleNode_getType(thread);
  {
    Arcadia_ValueStack_pushNatural8Value(thread, 0);
    Arcadia_superTypeConstructor(thread, _type, self);
  }
  if (Arcadia_ValueStack_getSize(thread) < 1 || 0 != Arcadia_ValueStack_getNatural8Value(thread, 0)) {
    Arcadia_Thread_setStatus(thread, Arcadia_Status_NumberOfArgumentsInvalid);
    Arcadia_Thread_jump(thread);
  }
  self->path = NULL;
  self->definitions = (Arcadia_List*)Arcadia_ArrayList_create(thread);
  Arcadia_Object_setType(thread, (Arcadia_Object*)self, _type);
  Arcadia_ValueStack_popValues(thread, 0 + 1);
}

static void
Arcadia_MIL_AST_ModuleNode_initializeDispatchImpl
  (
    Arcadia_Thread* thread,
    Arcadia_MIL_AST_ModuleNodeDispatch* self
  )
{ }

static void
Arcadia_MIL_AST_ModuleNode_visit
  (
    Arcadia_Thread* thread,
    Arcadia_MIL_AST_ModuleNode* self
  )
{
  if (self->path) {
    Arcadia_Object_visit(thread, (Arcadia_Object*)self->path);
  }
  Arcadia_Object_visit(thread, (Arcadia_Object*)self->definitions);
}

Arcadia_MIL_AST_ModuleNode*
Arcadia_MIL_AST_ModuleNode_create
  (
    Arcadia_Thread* thread
  )
{
  Arcadia_SizeValue oldValueStackSize = Arcadia_ValueStack_getSize(thread);
  Arcadia_ValueStack_pushNatural8Value(thread, 0);
  ARCADIA_CREATEOBJECT(Arcadia_MIL_AST_ModuleNode);
}

void
Arcadia_MIL_AST_ModuleNode_appendDefinition
  (
    Arcadia_Thread* thread,
    Arcadia_MIL_AST_ModuleNode* self,
    Arcadia_MIL_DefinitionAst* definition
  )
{
  Arcadia_List_insertBackObjectReferenceValue(thread, self->definitions, (Arcadia_ObjectReferenceValue)definition);
}

Arcadia_SizeValue
Arcadia_MIL_AST_ModuleNode_getNumberOfDefinitions
  (
    Arcadia_Thread* thread,
    Arcadia_MIL_AST_ModuleNode* self
  )
{ return Arcadia_Collection_getSize(thread, (Arcadia_Collection*)self->definitions); }

Arcadia_MIL_DefinitionAst*
Arcadia_MIL_AST_ModuleNode_getDefinitionAt
  (
    Arcadia_Thread* thread,
    Arcadia_MIL_AST_ModuleNode* self,
    Arcadia_SizeValue index
  )
{
  Arcadia_Value definitionAstValue = Arcadia_List_getAt(thread, self->definitions, index);
  Arcadia_MIL_DefinitionAst* definitionAst = Arcadia_Value_getObjectReferenceValue(&definitionAstValue);
  return definitionAst;
}
