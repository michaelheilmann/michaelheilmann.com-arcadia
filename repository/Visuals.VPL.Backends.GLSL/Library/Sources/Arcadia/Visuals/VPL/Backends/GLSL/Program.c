  ...
  )
{
  if (variableScalar->isInput) {
    Arcadia_ByteBuffer_append_pn(thread, target, u8"in", strlen(u8"in"));
  } else {
    Arcadia_ByteBuffer_append_pn(thread, target, u8"out", strlen(u8"out"));
  }
  Arcadia_ByteBuffer_append_pn(thread, target, u8" ", strlen(u8" "));
  Arcadia_ByteBuffer_append_pn(thread, target, Arcadia_String_getBytes(thread, variableScalar->type), Arcadia_String_getNumberOfBytes(thread, variableScalar->type));
  Arcadia_ByteBuffer_append_pn(thread, target, u8" ", strlen(u8" "));
  Arcadia_ByteBuffer_append_pn(thread, target, Arcadia_String_getBytes(thread, variableScalar->name), Arcadia_String_getNumberOfBytes(thread, variableScalar->name));
  Arcadia_ByteBuffer_append_pn(thread, target, u8";\n", strlen(u8";\n"));
}
  ...
  )
{
  ...
  Arcadia_ByteBuffer_append_pn(thread, target, u8"layout(std140) uniform ", strlen(u8"layout(std140) uniform "));
  Arcadia_ByteBuffer_append_pn(thread, target, Arcadia_String_getBytes(thread, name), Arcadia_String_getNumberOfBytes(thread, name));
  Arcadia_ByteBuffer_append_pn(thread, target, u8" {\n", strlen(u8" {\n"));
  for (Arcadia_SizeValue i = 0, n = Arcadia_Collection_getSize(thread, (Arcadia_Collection*)fields); i < n; ++i) {
    ...
    Arcadia_ByteBuffer_append_pn(thread, target, u8"  ", strlen(u8"  "));
    Arcadia_ByteBuffer_append_pn(thread, target, Arcadia_String_getBytes(thread, field->type), Arcadia_String_getNumberOfBytes(thread, field->type));
    Arcadia_ByteBuffer_append_pn(thread, target, u8" ", strlen(u8" "));
    Arcadia_ByteBuffer_append_pn(thread, target, Arcadia_String_getBytes(thread, field->name), Arcadia_String_getNumberOfBytes(thread, field->name));
    Arcadia_ByteBuffer_append_pn(thread, target, u8";\n", strlen(u8";\n"));
  }
  Arcadia_ByteBuffer_append_pn(thread, target, u8"}", strlen(u8"}"));
  Arcadia_String* temporary = Arcadia_String_createFromSize(thread, Arcadia_Value_getSizeValue(&id));
  Arcadia_ByteBuffer_append_pn(thread, target, u8"_", strlen(u8"_"));
  Arcadia_ByteBuffer_append_pn(thread, target, Arcadia_String_getBytes(thread, constantBlock->name), Arcadia_String_getNumberOfBytes(thread, constantBlock->name));
  Arcadia_ByteBuffer_append_pn(thread, target, Arcadia_String_getBytes(thread, temporary), Arcadia_String_getNumberOfBytes(thread, temporary));
  Arcadia_ByteBuffer_append_pn(thread, target, u8";\n", strlen(u8";\n"));
}
  ...
  )
{
  static const char* program =
    ...
    "}\n"
    ;
  Arcadia_ByteBuffer_append_pn(thread, target, program, strlen(program) + 1);
}
