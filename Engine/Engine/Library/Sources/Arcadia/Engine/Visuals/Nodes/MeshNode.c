  ...
  )
{
  ...
  if (self->backendContext) {
    if (self->meshResource) {
      Arcadia_Engine_Visuals_Implementation_Resource_unref(thread, (Arcadia_Engine_Visuals_Implementation_Resource*)self->meshResource);
      self->meshResource = NULL;
    }
    Arcadia_Object_unlock(thread, (Arcadia_Object*)self->backendContext);
    self->backendContext = NULL;
  ...
  )
{
  ...
  if (self->backendContext) {
    if (!self->meshResource) {
      ...
      Arcadia_Engine_Visuals_Implementation_VertexBufferResource* vertexBufferResource = NULL;
      vertexBufferResource = Arcadia_Engine_Visuals_BackendContext_createVertexBufferResource(thread, backendContext);
      ...
      Arcadia_Engine_Visuals_Implementation_Resource_ref(thread, (Arcadia_Engine_Visuals_Implementation_Resource*)self->meshResource);
      // (2.3) Set the mesh ambient color of the mesh resource.
      Arcadia_Engine_Visuals_Implementation_MeshResource_setMeshAmbientColor(thread, self->meshResource, ((Arcadia_Engine_Visuals_MeshNode*)self)->ambientColor);
    }
  }
}
