...
#if !defined(ARCADIA_VISUALS_WINDOWBACKEND_H_INCLUDED)
#define ARCADIA_VISUALS_WINDOWBACKEND_H_INCLUDED
...
// "Arcadia.Engine.Visuals.WindowBackend" is the ancestor class of all window backends.
...
// The window is put in "closed" state if it is destructed or by a successful call to "Arcadia.Engine.Visuals.WindowBackend.close".
Arcadia_declareObjectType(u8"Arcadia.Engine.Visuals.WindowBackend", Arcadia_Engine_Visuals_WindowBackend,
                          u8"Arcadia.Object");
...
struct Arcadia_Engine_Visuals_WindowBackend {
  ...
  struct {
    /// @brief #Arcadia_BooleanValue_True if the mouse is in the window. #Arcadia_BooleanValue_False.
    Arcadia_BooleanValue inWindow;
    /// @brief The last recorded position of the mouse pointer along the x-axis.
    /// This value is only meaningful if @a inWindow is #Arcadia_BooleanValue_True.
    Arcadia_Integer32Value oldx;
    /// @brief The last observed position of the mouse pointer along the y-axis.
    /// This value is only meaningful if @a inWindow is #Arcadia_BooleanValue_True.
    Arcadia_Integer32Value oldy;
  } mouse;
};
