  ...
  )
{
  ...
  errno = 0;
  // Open directory.
  Arcadia_String* path = Arcadia_ArgumentsValidation_getObjectReferenceValue(thread, &argumentValues[0], _Arcadia_FilePath_getType(thread));
  _self->dir = opendir(Arcadia_FilePath_toNative(thread, path));
  if (!_self->dir) {
    switch (errno) {
      case ENOENT: {
        errno = 0;
