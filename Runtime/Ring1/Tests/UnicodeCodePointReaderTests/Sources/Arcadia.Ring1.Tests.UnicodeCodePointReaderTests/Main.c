  ...
  )
{
  ...
  Arcadia_UnicodeCodePointReader* x = (Arcadia_UnicodeCodePointReader*)Arcadia_ByteReader_UnicodeCodePointReader_create(thread, y);
    ...
    while (resultIndex < numberOfResults) {
      ...
      if (result->error) {
        if (Arcadia_UnicodeCodePointReader_hasValue(thread, x)) {
          Arcadia_Thread_setStatus(thread, Arcadia_Status_TestFailed);
          Arcadia_Thread_jump(thread);
        }
        if (!Arcadia_UnicodeCodePointReader_hasError(thread, x)) {
          Arcadia_Thread_setStatus(thread, Arcadia_Status_TestFailed);
          Arcadia_Thread_jump(thread);
        }
      } else {
...
static void
Arcadia_Ring1_Tests_UnicodeCodePointReaderTest1
  (
    Arcadia_Thread* thread
  )
{
...
static void
Arcadia_Ring1_Tests_UnicodeCodePointReaderTest2
(
  Arcadia_Thread* thread
) {
  {
...
static void
Arcadia_Ring1_Tests_UnicodeCodePointReaderTest3
  (
    Arcadia_Thread* thread
  )
{
...
{
  if (!Arcadia_Tests_safeExecute(&Arcadia_Ring1_Tests_UnicodeCodePointReaderTest1)) {
    return EXIT_FAILURE;
  }
  if (!Arcadia_Tests_safeExecute(&Arcadia_Ring1_Tests_UnicodeCodePointReaderTest2)) {
    return EXIT_FAILURE;
  }
  if (!Arcadia_Tests_safeExecute(&Arcadia_Ring1_Tests_UnicodeCodePointReaderTest3)) {
    return EXIT_FAILURE;
  }
  return EXIT_SUCCESS;
}
