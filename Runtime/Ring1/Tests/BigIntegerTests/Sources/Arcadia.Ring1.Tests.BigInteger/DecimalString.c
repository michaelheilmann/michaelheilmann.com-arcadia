  ...
  )
{
  ...
  Arcadia_Tests_assertTrue(thread, Arcadia_Integer32Value_Literal(0) == Arcadia_Process_compareMemory(Arcadia_Thread_getProcess(thread), bytes, s->bytes, numberOfBytes));
}
