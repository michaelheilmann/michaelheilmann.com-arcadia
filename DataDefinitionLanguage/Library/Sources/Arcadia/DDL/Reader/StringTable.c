...
#include "Arcadia/DDL/Reader/StringTable.h"

#include "Arcadia/DDL/Include.h"

static void
Arcadia_DDL_StringTable_constructImpl
  (
    Arcadia_Thread* thread,
    Arcadia_DDL_StringTable* self
  );

static void
Arcadia_DDL_StringTable_maybeResize_nojump
  (
    Arcadia_Thread* thread,
    Arcadia_DDL_StringTable* self
  );
...
Arcadia_DDL_StringTable_hashBytes
  (
    Arcadia_Thread* thread,
...
static void
Arcadia_DDL_StringTable_destruct
  (
    Arcadia_Thread* thread,
    Arcadia_DDL_StringTable* self
  );
...
static const Arcadia_ObjectType_Operations _objectTypeOperations = {
  .construct = (Arcadia_Object_ConstructorCallbackFunction*)&Arcadia_DDL_StringTable_constructImpl,
  .destruct = (Arcadia_Object_DestructorCallbackFunction*)&Arcadia_DDL_StringTable_destruct,
  .visit = (Arcadia_Object_VisitCallbackFunction*)&Arcadia_DDL_StringTable_visit,
};
...
Arcadia_defineObjectType(u8"Arcadia.DDL.StringTable", Arcadia_DDL_StringTable,
...
{
  Arcadia_TypeValue _type = _Arcadia_DDL_StringTable_getType(thread);
  {
  ...
  self->buckets = Arcadia_Memory_allocateUnmanaged(thread, sizeof(Arcadia_DDL_StringTable_Node*) * g_defaultCapacity);
  ...
  )
{
  if (self->size >= self->capacity) {
    ...
    if (Arcadia_JumpTarget_save(&jumpTarget)) {
      Arcadia_SizeValue maximumCapacity = Arcadia_SizeValue_Maximum / sizeof(Arcadia_DDL_StringTable_Node*);
      ...
      Arcadia_DDL_StringTable_Node** oldBuckets = self->buckets;
      Arcadia_DDL_StringTable_Node** newBuckets = NULL;
      newBuckets = Arcadia_Memory_allocateUnmanaged(thread, sizeof(Arcadia_DDL_StringTable_Node*) * newCapacity);
      ...
      for (Arcadia_SizeValue oldIndex = 0, n = oldCapacity; oldIndex < n; ++oldIndex) {
        while (oldBuckets[oldIndex]) {
          Arcadia_DDL_StringTable_Node* node = oldBuckets[oldIndex];
  ...
  )
{
  for (Arcadia_SizeValue i = 0, n = self->capacity; i < n; ++i) {
    Arcadia_DDL_StringTable_Node* node = self->buckets[i];
    while (node) {
...
Arcadia_DDL_StringTable_create
  (
    Arcadia_Thread* thread
  )
{
  ...
  Arcadia_ValueStack_pushNatural8Value(thread, 0);
  ARCADIA_CREATEOBJECT(Arcadia_DDL_StringTable);
}

Arcadia_String*
Arcadia_DDL_StringTable_getOrCreateString
  (
    Arcadia_Thread* thread,
    Arcadia_DDL_StringTable* self,
  ...
  )
{
  ...
  Arcadia_SizeValue hash = Arcadia_DDL_StringTable_hashBytes(thread, bytes, numberOfBytes);
  Arcadia_SizeValue index = hash % self->capacity;
  for (Arcadia_DDL_StringTable_Node* node = self->buckets[index]; NULL != node; node = node->next) {
  ...
  Arcadia_DDL_StringTable_Node* node = Arcadia_Memory_allocateUnmanaged(thread, sizeof(Arcadia_DDL_StringTable_Node));
  node->string = string;
  node->hash = Arcadia_DDL_StringTable_hashBytes(thread, Arcadia_String_getBytes(thread, string), Arcadia_String_getNumberOfBytes(thread, string));
  node->next = self->buckets[index];
  self->buckets[index] = node;
  self->size++;

  Arcadia_DDL_StringTable_maybeResize_nojump(thread, self);
