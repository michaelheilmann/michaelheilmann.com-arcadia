...
static const Arcadia_ObjectType_Operations _Arcadia_DDLS_ScalarNode_objectTypeOperations = {
  ...
  .initializeDispatch = (Arcadia_ObjectDispatch_InitializeCallbackFunction*)&Arcadia_DDLS_ScalarNode_initializeDispatchImpl,
};
...
{
  Arcadia_EnterConstructor(Arcadia_DDLS_ScalarNode);
  //
  {
  ...
  self->name = NULL;
  //
  Arcadia_LeaveConstructor(Arcadia_DDLS_ScalarNode);
}
...
{
  _Arcadia_BeginCreate(Arcadia_DDLS_ScalarNode);
  Arcadia_ValueStack_pushNatural8Value(thread, 0);
  _Arcadia_EndCreate(Arcadia_DDLS_ScalarNode);
}
